...
function excluirSelecionados(idsParaExcluir) {
  ...
  var lista = JSON.parse(localStorage.getItem("listaMedicamentosStorage"));

  const novaLista = lista.filter((item) => {
    return !idsParaExcluir.includes(String(item.id));
  });

  localStorage.setItem("listaMedicamentosStorage", JSON.stringify(novaLista));
  carregaItens();
}
...
export function selecionaItenASerExcluido() {
  conteudoPrincipal.addEventListener("click", (e) => {
    if (e.target.id === "btn-excluir-selecionados") {
      ...
      if (!listaMedicamentosDiv) return;

      const checkboxesMarcadas = listaMedicamentosDiv.querySelectorAll(".item-para-excluir:checked");
...
export function carregaItens(searchTerm = "") {
  ...
  if (!listaMedicamentosDiv) {
    return;
  }

  if (localStorage.getItem("listaMedicamentosStorage")) {
    var listaCompleta = JSON.parse(localStorage.getItem("listaMedicamentosStorage"));
    ...
    html += "<td></td>";
    html += "<td>ID</td>";
    html += "<td>Nome do medicamento</td>";
    html += "</tr>";
    ...
    html += "</table>";
    listaMedicamentosDiv.innerHTML = html;
  } else {
    var obj = {id: 1, nome: "teste"};
    var lista = [];
    lista.push(obj);
    localStorage.setItem("listaMedicamentosStorage", JSON.stringify(lista));
    carregaItens();
  }
}
...
export function adicionaListenerDeBusca() {
  conteudoPrincipal.addEventListener("keyup", (e) => {
    if (e.target.id === "input-busca-medicamentos") {
...
function armazenar() {
  var listaClientes = JSON.parse(localStorage.getItem("listaMedicamentosStorage"));
  const novoId = gerarProximoId(listaClientes);

  var obj = {id: novoId, nome: ""};
  obj.nome = document.getElementById("nome-medicamento").value;
  listaClientes.push(obj);
  localStorage.setItem("listaMedicamentosStorage", JSON.stringify(listaClientes));
}

export function cadastrarMedicamentos() {
  conteudoPrincipal.addEventListener("click", (e) => {
    if (e.target.id === "enviar-informacoes-medicamento") {
      e.preventDefault();

      armazenar();
      carregarPaginaHtml("html/medicamentos.html");
    }
  });
}
